...
const userLogin = async ({ phone }: User): Promise<{ status: string, message: string, token: string | null, userId?: string }> => {
  try {
    if (!phone) return { status: "error", message: "Data not found.", token: null };
    ...
    if (token) return { status: "success", message: "User logged in.", token, userId: user._id };
